...
const Announcements = () => {
  ...
  const handleImagePick = (e) => {
    ...
    if (!file) return;
    if (!file.type.startsWith('image/')) { flash('danger', 'Please choose an image file.'); return; }
    if (file.size > 5 * 1024 * 1024) { flash('danger', 'Image must be under 5MB.'); return; }
    setImageFile(file);
  ...
  const uploadImage = async () => {
    ...
    const { error } = await supabase.storage.from('announcements').upload(path, imageFile, { upsert: false });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = { ...form, image_url: imageUrl || null, publish_date: form.publishDate || null, event_date: form.eventDate || null, pinned: form.pinned ? 1 : 0 };
      if (editingId) {
      ...
      flash('danger', err.message || 'Failed to save announcement.');
    } finally {
      setUploading(false);
    }
  };
        ...
        <Card.Body className="p-3">
          <div className="d-flex flex-wrap align-items-center gap-2 mb-2">
            ...
            <div className="d-flex align-items-center gap-2 flex-grow-1 flex-md-grow-0 ms-md-auto">
              <InputGroup size="sm" className="flex-grow-1" style={{ minWidth: 0, maxWidth: 280 }}>
              ...
              >
                <Card.Body className="p-3 p-md-4">
                  <div className="d-flex flex-wrap align-items-start gap-3">
                    ...
                    <div className="flex-grow-1" style={{ minWidth: 0 }}>
                      ...
                      {a.image_url && (
                        <img
                          src={a.image_url}
                          alt=""
                          style={{ maxWidth: '100%', width: 200, height: 110, objectFit: 'cover', borderRadius: 8, border: '1px solid #e5e7eb', marginBottom: 8, display: 'block' }}
                        />
                      )}
                    ...
                    <div className="d-flex flex-row flex-wrap gap-1 flex-shrink-0 ms-md-auto">
                      <Button
      ...
      <Modal show={!!showView} onHide={() => setShowView(null)} centered size="lg">
        ...
        {showView && (
          <Modal.Body>
            ...
            {showView.image_url && (
              <img
                src={showView.image_url}
                alt=""
                style={{ width: '100%', maxHeight: 360, objectFit: 'cover', borderRadius: 12, border: '1px solid #e5e7eb', marginBottom: 16 }}
              />
            )}
      ...
      <Modal show={showForm} onHide={closeForm} centered size="lg">
        ...
        <Form onSubmit={handleSubmit}>
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>Picture (optional)</Form.Label>
              {imagePreview ? (
                <div style={{ position: 'relative', width: 'fit-content' }}>
                  <img
                    src={imagePreview}
                    alt="preview"
                    style={{ maxWidth: '100%', maxHeight: 180, borderRadius: 10, border: '1px solid #e5e7eb', objectFit: 'cover' }}
                  />
                  <Button
                    size="sm" variant="light"
                    onClick={removeImage}
                    className="d-flex align-items-center justify-content-center border-0 p-0"
                    style={{ position: 'absolute', top: 6, right: 6, width: 26, height: 26, borderRadius: '50%', backgroundColor: 'rgba(0,0,0,0.55)' }}
                    title="Remove image"
                  >
                    <X size={15} color="#fff" />
                  </Button>
                ...
                <label
                  ...
                  style={{ border: '2px dashed #cbd5e1', borderRadius: 10, padding: '18px', cursor: 'pointer', color: '#64748b', backgroundColor: '#f8fafc' }}
                >
                  <ImagePlus size={22} color="#0d9488" />
                  <span style={{ fontSize: 12, fontWeight: 600 }}>Click to upload a picture</span>
                  <span style={{ fontSize: 11, color: '#94a3b8' }}>JPG / PNG, up to 5MB</span>
          ...
          <Modal.Footer>
            <Button variant="light" size="sm" onClick={closeForm} disabled={uploading}>Cancel</Button>
            <Button
              type="submit"
              size="sm"
              disabled={uploading}
            ...
            >
              {uploading ? 'Saving…' : editingId ? 'Save Changes' : 'Create Announcement'}
            </Button>
          </Modal.Footer>
        </Form>
      </Modal>
