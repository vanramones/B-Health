...
const StatusPill = ({ status }) => {
  ...
  return (
    <span style={{ display: 'inline-block', backgroundColor: sv.bg, color: sv.color, border: `1px solid ${sv.border}`, fontWeight: 700, fontSize: 11, padding: '4px 12px', borderRadius: 999, letterSpacing: '0.02em', lineHeight: 1.4 }}>
      {sv.label}
    </span>
  );
};
...
const TypePill = ({ type }) => {
  ...
  return (
    <span className="d-inline-flex align-items-center gap-1" style={{ backgroundColor: tv.bg, color: tv.color, fontWeight: 600, fontSize: 11, padding: '4px 10px', borderRadius: 8 }}>
      {tv.icon} {type}
    </span>
  );
};
...
const HealthRecords = () => {
  const { items, loading, createItem, updateItem, deleteItem, refresh } = useCrud('/health-records');
  ...
  React.useEffect(() => {
    const channel = supabase
      .channel('admin-health-records-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'health_records' }, handleRealtime)
      .subscribe();
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((r) => {
      ...
      const matchQuery  = !q || r.patient.toLowerCase().includes(q) || r.diagnosis.toLowerCase().includes(q) || r.doctor.toLowerCase().includes(q) || r.type.toLowerCase().includes(q);
      return matchStatus && matchQuery;
    });
  }, [items, filter, query]);

  const openAdd = () => { setEditingId(null); setForm({ ...emptyForm, date: new Date().toISOString().slice(0, 10) }); setShowForm(true); };
  const openEdit = (r) => { setEditingId(r.id); setForm({ ...r }); setShowForm(true); };
  ...
  const handleSave = async (e) => {
    ...
    } catch (err) { setFormError(err.message || 'Failed to save record'); }
  };

  const handleDelete = async () => {
    if (confirmDelete) { await deleteItem(confirmDelete.id); setConfirmDelete(null); }
  };
  ...
  return (
    <div className="p-4" style={{ backgroundColor: '#f8fafc' }}>
          ...
          <div className="d-flex flex-wrap align-items-center gap-2">
            ...
            <div className="ms-auto d-flex align-items-center gap-2">
              <InputGroup size="sm" style={{ width: 280 }}>
                ...
                <Form.Control className="bg-light border-start-0" placeholder="Search patient, diagnosis, doctor..." value={query} onChange={(e) => setQuery(e.target.value)} style={{ fontSize: 13 }} />
              </InputGroup>
              <Button size="sm" onClick={openAdd} className="d-flex align-items-center gap-1 border-0" style={{ backgroundColor: '#16a34a', fontWeight: 500, fontSize: 12 }}>
            ...
            <Table hover className="mb-0 align-middle">
              <thead style={{ backgroundColor: '#f9fafb' }}>
                <tr style={{ fontSize: 11, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                  <th className="px-4 py-3">Patient</th><th className="py-3">Type</th><th className="py-3">Diagnosis</th>
                  <th className="py-3">Doctor</th><th className="py-3">Date</th><th className="py-3">Status</th>
                  <th className="py-3 text-end pe-4">Actions</th>
                </tr>
              </thead>
              <tbody style={{ fontSize: 13 }}>
                {filtered.length === 0 && <tr><td colSpan={7} className="text-center py-5" style={{ color: '#9ca3af' }}>No health records found.</td></tr>}
                {filtered.map((r) => (
                  <tr key={r.id} onClick={() => setShowView(r)} style={{ cursor: 'pointer' }}>
                    <td className="px-4 py-3">
                      <div className="d-flex align-items-center gap-2">
                        <div className="d-flex align-items-center justify-content-center rounded-circle text-white fw-bold" style={{ width: 32, height: 32, backgroundColor: '#16a34a', fontSize: 11 }}>
                    ...
                    <td className="py-3 text-end pe-4" onClick={(e) => e.stopPropagation()}>
                      <div className="d-flex justify-content-end gap-1">
                        <Button size="sm" variant="light" className="border-0 p-1 px-2" onClick={() => setShowView(r)}><Eye size={14} color="#6b7280" /></Button>
                        <Button size="sm" variant="light" className="border-0 p-1 px-2" onClick={() => openEdit(r)}><Edit3 size={14} color="#1d4ed8" /></Button>
                        <Button size="sm" variant="light" className="border-0 p-1 px-2" onClick={() => setConfirmDelete(r)}><Trash2 size={14} color="#dc2626" /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <div className="d-md-none">
        {filtered.length === 0 ? (
          <Card className="border rounded-4"><Card.Body className="p-4 text-center" style={{ color: '#9ca3af' }}><FileText size={32} className="mb-2" /><div style={{ fontSize: 14 }}>No health records found.</div></Card.Body></Card>
        ) : (
          <Row className="g-2">
            {filtered.map((r) => {
              ...
              return (
                <Col key={r.id} xs={12}>
                  <Card className="border rounded-4 bh-card-hover" onClick={() => setShowView(r)} style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)', borderLeft: `4px solid ${tv.color}`, cursor: 'pointer' }}>
                    <Card.Body className="p-3">
                      <div className="d-flex align-items-start gap-3">
                        <div className="d-flex align-items-center justify-content-center rounded-3 flex-shrink-0" style={{ width: 44, height: 44, backgroundColor: tv.bg, color: tv.color }}>
                        ...
                        <div className="flex-grow-1" style={{ minWidth: 0 }}>
                          ...
                          <div className="d-flex flex-wrap align-items-center gap-2 mb-2"><TypePill type={r.type} /></div>
                          <div style={{ fontSize: 13, color: '#374151', fontWeight: 500, marginBottom: 4 }}>{r.diagnosis}</div>
                          <div className="d-flex flex-wrap gap-3" style={{ fontSize: 12, color: '#6b7280' }}>
                            <span className="d-flex align-items-center gap-1"><User size={12} /> {r.doctor || '—'}</span>
                            <span className="d-flex align-items-center gap-1"><Calendar size={12} /> {r.date}</span>
                          </div>
                          {r.notes && <div className="mt-2 p-2 rounded-2 text-truncate" style={{ backgroundColor: '#f9fafb', fontSize: 11, color: '#6b7280' }}>{r.notes}</div>}
                        </div>
                      </div>
                      <div className="d-flex gap-2 mt-3 pt-2" style={{ borderTop: '1px solid #f3f4f6' }} onClick={(e) => e.stopPropagation()}>
                        <Button size="sm" variant="light" className="flex-grow-1 border-0 d-flex align-items-center justify-content-center gap-1" onClick={() => setShowView(r)} style={{ fontSize: 12 }}><Eye size={13} color="#6b7280" /> View</Button>
                        <Button size="sm" variant="light" className="flex-grow-1 border-0 d-flex align-items-center justify-content-center gap-1" onClick={() => openEdit(r)} style={{ fontSize: 12 }}><Edit3 size={13} color="#1d4ed8" /> Edit</Button>
                        <Button size="sm" variant="light" className="flex-grow-1 border-0 d-flex align-items-center justify-content-center gap-1" onClick={() => setConfirmDelete(r)} style={{ fontSize: 12 }}><Trash2 size={13} color="#dc2626" /> Delete</Button>
                      </div>
                    </Card.Body>
                  </Card>
                </Col>
      ...
      <Modal show={showForm} onHide={() => setShowForm(false)} centered>
        <Modal.Header closeButton><Modal.Title style={{ fontSize: 16 }}>{editingId ? 'Edit Health Record' : 'Add Health Record'}</Modal.Title></Modal.Header>
        <Form onSubmit={handleSave}>
          <Modal.Body>
            {formError && <div style={{ padding: '10px 12px', marginBottom: 16, backgroundColor: '#fee2e2', border: '1px solid #fecaca', borderRadius: 8, fontSize: 13, color: '#dc2626' }}>{formError}</div>}
            <Form.Group className="mb-3">
              <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>Registered User</Form.Label>
              <div style={{ position: 'relative', marginBottom: 8 }}>
                <Form.Control size="sm" placeholder="Click arrow or search to select user..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)} onFocus={() => setShowUserDropdown(true)} style={{ paddingRight: '32px', cursor: 'pointer' }} />
                <div onClick={() => setShowUserDropdown(!showUserDropdown)} style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <ChevronDown size={18} style={{ color: '#9ca3af', transition: 'transform 0.2s ease', transform: showUserDropdown ? 'rotate(180deg)' : 'rotate(0deg)' }} />
                </div>
              </div>
              {showUserDropdown && (
                <div style={{ border: '1px solid #d1d5db', borderRadius: 6, maxHeight: 250, overflowY: 'auto', backgroundColor: '#fff', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>
                  {users.length > 0 ? (
                    (userSearch ? filteredUsers : users).map((u) => (
                      <div key={u.id} onClick={() => { setForm({ ...form, patient: u.full_name }); setUserSearch(''); setShowUserDropdown(false); }}
                        style={{ padding: '10px 12px', cursor: 'pointer', borderBottom: '1px solid #f3f4f6', fontSize: 13, color: form.patient === u.full_name ? '#1d4ed8' : '#374151', backgroundColor: form.patient === u.full_name ? '#eff6ff' : '#fff', fontWeight: form.patient === u.full_name ? 600 : 400 }}
                    ...
                    <div style={{ padding: '8px 12px', fontSize: 13, color: '#9ca3af', backgroundColor: '#f9fafb' }}>Loading users...</div>
                  )}
                </div>
              )}
              {form.patient && (
                <div style={{ marginTop: 8, padding: '10px 12px', backgroundColor: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 6, fontSize: 13, color: '#1d4ed8', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <User size={14} /> Selected: <strong>{form.patient}</strong>
                </div>
              )}
            </Form.Group>
            <Row className="g-2 mb-3">
              <Col xs={6}>
                ...
                <Form.Select size="sm" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>
                  <option>Consultation</option><option>Vaccination</option><option>Prenatal</option><option>Lab Result</option><option>Prescription</option>
                </Form.Select>
              </Col>
              <Col xs={6}>
                ...
                <Form.Select size="sm" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
                  <option value="ongoing">Ongoing</option><option value="follow-up">Follow-up</option><option value="closed">Closed</option>
                </Form.Select>
              </Col>
            </Row>
            <Form.Group className="mb-3">
          ...
          <Modal.Footer>
            ...
            <Button type="submit" size="sm" className="border-0" style={{ backgroundColor: '#16a34a' }}>{editingId ? 'Save Changes' : 'Add Record'}</Button>
          </Modal.Footer>
        </Form>
      </Modal>
      ...
      <Modal show={!!confirmDelete} onHide={() => setConfirmDelete(null)} centered size="sm">
        ...
        <Modal.Body style={{ fontSize: 13 }}>Delete this record for <strong>{confirmDelete?.patient}</strong>? This action cannot be undone.</Modal.Body>
        <Modal.Footer>
