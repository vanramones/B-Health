...
const Residents = () => {
  const { items, loading, createItem, updateItem, deleteItem, refresh } = useCrud('/residents');
  ...
  React.useEffect(() => {
    const channel = supabase
      .channel('admin-residents-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'residents' }, handleRealtime)
      .subscribe();
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((r) => {
      ...
      const matchQuery  = !q || r.name.toLowerCase().includes(q) || r.address.toLowerCase().includes(q) || r.condition.toLowerCase().includes(q);
      return matchStatus && matchQuery;
    });
  }, [items, filter, query]);

  const openAdd = () => { setEditingId(null); setForm(emptyForm); setShowForm(true); };
  const openEdit = (r) => { setEditingId(r.id); setForm({ ...r, age: String(r.age) }); setShowForm(true); };
  ...
  return (
    <div className="p-4" style={{ backgroundColor: '#f8fafc' }}>
        ...
        <Card.Body className="p-3">
          <div className="d-flex flex-wrap align-items-center gap-2">
            ...
            <div className="ms-auto d-flex align-items-center gap-2 mt-2 mt-md-0">
              ...
              <Button size="sm" onClick={openAdd} className="d-flex align-items-center gap-1 border-0 flex-shrink-0" style={{ backgroundColor: '#16a34a', fontWeight: 500, fontSize: 12 }}>
          ...
          <div className="table-responsive-wrapper">
            <Table hover className="mb-0 align-middle">
              ...
              <tbody style={{ fontSize: 13 }}>
                {filtered.length === 0 && <tr><td colSpan={7} className="text-center py-5" style={{ color: '#9ca3af' }}>No residents found.</td></tr>}
                {filtered.map((r) => (
                  <tr key={r.id} onClick={() => setShowView(r)} style={{ cursor: 'pointer' }}>
                    <td className="px-4 py-3">
                      <div className="d-flex align-items-center gap-2">
                        <div className="d-flex align-items-center justify-content-center rounded-circle text-white fw-bold"
                          style={{ width: 32, height: 32, backgroundColor: r.gender === 'Female' ? '#ec4899' : '#3b82f6', fontSize: 11 }}>
                    ...
                    <td className="py-3 text-end pe-4">
                      <div className="d-flex justify-content-end gap-1">
                        <Button size="sm" variant="light" className="border-0 p-1 px-2" onClick={(e) => { e.stopPropagation(); setShowView(r); }} title="View"><Eye size={14} color="#6b7280" /></Button>
                        <Button size="sm" variant="light" className="border-0 p-1 px-2" onClick={(e) => { e.stopPropagation(); openEdit(r); }} title="Edit"><Edit3 size={14} color="#1d4ed8" /></Button>
                        <Button size="sm" variant="light" className="border-0 p-1 px-2" onClick={(e) => { e.stopPropagation(); setConfirmDelete(r); }} title="Delete"><Trash2 size={14} color="#dc2626" /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <div className="d-md-none">
        {filtered.length === 0 ? (
          <Card className="border rounded-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <Card.Body className="p-4 text-center" style={{ color: '#9ca3af' }}><Users size={32} className="mb-2" /><div style={{ fontSize: 14 }}>No residents found.</div></Card.Body>
          </Card>
        ) : (
          <Row className="g-2">
            {filtered.map((r) => (
              <Col key={r.id} xs={12}>
                <Card className="border rounded-4 bh-card-hover" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)', cursor: 'pointer' }} onClick={() => setShowView(r)}>
                  <Card.Body className="p-3">
                    <div className="d-flex align-items-start gap-3">
                      <div className="d-flex align-items-center justify-content-center rounded-circle text-white fw-bold flex-shrink-0"
                        style={{ width: 44, height: 44, backgroundColor: r.gender === 'Female' ? '#ec4899' : '#3b82f6', fontSize: 13 }}>
                        {r.name.split(' ').map((p) => p[0]).slice(0, 2).join('')}
                      </div>
                      <div className="flex-grow-1" style={{ minWidth: 0 }}>
                        ...
                        <div className="d-flex flex-wrap gap-2 mb-2" style={{ fontSize: 12, color: '#6b7280' }}>
                          <span>{r.age} yrs</span><span>•</span><span>{r.gender}</span><span>•</span><span>{r.condition}</span>
                        </div>
                        <div className="d-flex align-items-center gap-1 mb-2" style={{ fontSize: 12, color: '#6b7280' }}><MapPin size={12} /><span className="text-truncate">{r.address}</span></div>
                        {r.contact && <div className="d-flex align-items-center gap-1" style={{ fontSize: 12, color: '#6b7280' }}><Phone size={12} /><span>{r.contact}</span></div>}
                      </div>
                    </div>
                    <div className="d-flex gap-2 mt-3 pt-2" style={{ borderTop: '1px solid #f3f4f6' }}>
                      <Button size="sm" variant="light" className="flex-grow-1 border-0 d-flex align-items-center justify-content-center gap-1" onClick={(e) => { e.stopPropagation(); setShowView(r); }} style={{ fontSize: 12 }}><Eye size={13} color="#6b7280" /> View</Button>
                      <Button size="sm" variant="light" className="flex-grow-1 border-0 d-flex align-items-center justify-content-center gap-1" onClick={(e) => { e.stopPropagation(); openEdit(r); }} style={{ fontSize: 12 }}><Edit3 size={13} color="#1d4ed8" /> Edit</Button>
                      <Button size="sm" variant="light" className="flex-grow-1 border-0 d-flex align-items-center justify-content-center gap-1" onClick={(e) => { e.stopPropagation(); setConfirmDelete(r); }} style={{ fontSize: 12 }}><Trash2 size={13} color="#dc2626" /> Delete</Button>
                    </div>
                  </Card.Body>
                </Card>
              </Col>
      ...
      <Modal show={showForm} onHide={() => setShowForm(false)} centered>
        <Modal.Header closeButton><Modal.Title style={{ fontSize: 16 }}>{editingId ? 'Edit Resident' : 'Add Resident'}</Modal.Title></Modal.Header>
        <Form onSubmit={handleSave}>
          <Modal.Body>
            ...
            <Row className="g-2 mb-3">
              ...
              <Col xs={4}>
                ...
                <Form.Select size="sm" value={form.gender} onChange={(e) => setForm({ ...form, gender: e.target.value })}><option>Male</option><option>Female</option></Form.Select>
              </Col>
              <Col xs={4}>
                ...
                <Form.Select size="sm" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
                  <option value="active">Active</option><option value="monitoring">Monitoring</option><option value="inactive">Inactive</option>
                </Form.Select>
              </Col>
            </Row>
            <Form.Group className="mb-3">
            ...
            <Row className="g-2">
              ...
              <Col xs={6}>
                ...
                <Form.Select size="sm" value={form.condition} onChange={(e) => setForm({ ...form, condition: e.target.value })}>
                  <option>Healthy</option><option>Pediatric</option><option>Pregnant</option><option>Hypertension</option><option>Diabetes</option><option>Senior Care</option><option>Other</option>
                </Form.Select>
              </Col>
            </Row>
          </Modal.Body>
          <Modal.Footer>
            ...
            <Button type="submit" size="sm" className="border-0" style={{ backgroundColor: '#16a34a' }}>{editingId ? 'Save Changes' : 'Add Resident'}</Button>
          </Modal.Footer>
        </Form>
      </Modal>
      ...
      <Modal show={!!showView} onHide={() => setShowView(null)} centered>
        <Modal.Header closeButton><Modal.Title style={{ fontSize: 16 }}>Resident Profile</Modal.Title></Modal.Header>
        {showView && (
          <Modal.Body>
            <div className="d-flex align-items-center gap-3 mb-3">
              <div className="d-flex align-items-center justify-content-center rounded-circle text-white fw-bold"
                style={{ width: 52, height: 52, backgroundColor: showView.gender === 'Female' ? '#ec4899' : '#3b82f6', fontSize: 16 }}>
                {showView.name.split(' ').map((p) => p[0]).slice(0, 2).join('')}
              </div>
              <div>
                <div className="fw-bold" style={{ fontSize: 16 }}>{showView.name}</div>
                <div style={{ fontSize: 12, color: '#6b7280' }}>{showView.age} years old &middot; {showView.gender}</div>
              </div>
      ...
      <Modal show={!!confirmDelete} onHide={() => setConfirmDelete(null)} centered size="sm">
        <Modal.Header closeButton><Modal.Title style={{ fontSize: 15 }}>Delete Resident?</Modal.Title></Modal.Header>
        <Modal.Body style={{ fontSize: 13 }}>Are you sure you want to delete <strong>{confirmDelete?.name}</strong>? This action cannot be undone.</Modal.Body>
        <Modal.Footer>
