...
const Services = () => {
  ...
  const { items: appointments } = useCrud('/appointments');
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((s) => {
      ...
      const matchStatus = statusFilter === 'all' || (statusFilter === 'active' && s.active);
      const matchQuery =
        !q ||
  ...
  // Realistic beneficiaries: distinct residents with a COMPLETED appointment per service
  const benefByService = useMemo(() => {
    const map = {};
    (appointments || []).forEach((a) => {
      if (a.status !== 'completed' || !a.service) return;
      const key = String(a.resident_id || a.user_id || a.name || a.id);
      (map[a.service] || (map[a.service] = new Set())).add(key);
    });
    const counts = {};
    Object.keys(map).forEach((k) => { counts[k] = map[k].size; });
    return counts;
  }, [appointments]);
  const benefCount = (name) => benefByService[name] || 0;
  const totalBeneficiaries = useMemo(
    () => items.reduce((s, x) => s + benefCount(x.name), 0),
    [items, benefByService],
  );
        ...
        {showView && (
          <Modal.Body>
            ...
            <div className="d-flex flex-column gap-2" style={{ fontSize: 13 }}>
              <div className="d-flex align-items-center gap-2"><Users size={14} color="#6b7280" /> <strong>Beneficiaries:</strong> {benefCount(showView.name)} <span style={{ fontSize: 11, color: '#9ca3af' }}>(completed appointments)</span></div>
      ...
      <Modal show={showForm} onHide={closeForm} centered>
        ...
        <Form onSubmit={handleSave}>
          <Modal.Body>
            ...
            <Row className="g-2 mb-3">
              <Col xs={6}>
                <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>Beneficiaries (auto)</Form.Label>
                <Form.Control size="sm" readOnly disabled value={editingId ? benefCount(form.name) : 0} />
                <Form.Text style={{ fontSize: 10, color: '#9ca3af' }}>Auto-counted from completed appointments.</Form.Text>
              </Col>
              <Col xs={6}>
