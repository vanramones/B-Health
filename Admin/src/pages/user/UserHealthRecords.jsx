  ...
  return (
    <div>
      ...
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        <div>
          <h4 className="fw-bold mb-1" style={{ color: '#111827' }}>{t('myHealthRecords')}</h4>
                  ...
                  return (
                    <Button
                      ...
                      style={{
                        ...
                        color: filter === type ? (tc?.color || '#1d4ed8') : '#374151',
                        fontWeight: 600,
                        fontSize: 12,
                      }}
                    >
      ...
      {loading ? (
        ...
        <Card className="border rounded-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <Card.Body className="p-5 text-center" style={{ color: '#9ca3af' }}>
            ...
            <div style={{ fontSize: 14 }}>{t('noRecords')}</div>
          </Card.Body>
        </Card>
      ) : (
                          ...
                          <div className="d-flex align-items-center gap-2 mb-1">
                            ...
                            {record.prescription && (
                              <Badge style={{ backgroundColor: '#fef3c7', color: '#b45309', fontSize: 10 }}>
                                <Pill size={10} className="me-1" /> {t('hasPrescription')}
                              </Badge>
                            )}
                          </div>
        ...
        {showView && (
          <>
            ...
            <Modal.Body>
                    ...
                    <>
                      ...
                      {showView.prescription && (
                        <Badge style={{ backgroundColor: '#fef3c7', color: '#b45309', fontSize: 11 }}>
                          <Pill size={12} className="me-1" /> {t('hasPrescription')}
                        </Badge>
                      )}
                    </>
                  );
              ...
              <Row className="g-4">
                <Col xs={12}>
                  <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, marginBottom: 4 }}>{t('diagnosis').toUpperCase()}</div>
                  <div className="fw-bold" style={{ fontSize: 18, color: '#111827' }}>{showView.diagnosis}</div>
                </Col>
                <Col xs={12} md={6}>
                  <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, marginBottom: 4 }}>{t('doctor').toUpperCase()}</div>
                  <div style={{ fontSize: 14, color: '#374151' }}>{showView.doctor}</div>
                </Col>
                <Col xs={12} md={6}>
                  <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, marginBottom: 4 }}>{t('date').toUpperCase()}</div>
                  <div style={{ fontSize: 14, color: '#374151' }}>
                    {new Date(showView.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}
                  </div>
                </Col>
                <Col xs={12}>
                  <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, marginBottom: 4 }}>{t('notesRemarks').toUpperCase()}</div>
                  <div
