   import React, { useState, useEffect, useCallback, useRef } from 'react';
import { Card, Row, Col, Button, Form, Badge, Modal } from 'react-bootstrap';
import { useUserAuth, userApi } from '../../context/UserAuthContext';
import { useLanguage } from '../../context/LanguageContext';
import { supabase } from '../../config/supabase';
import {
  User, Mail, Phone, MapPin, Calendar, Heart, Shield,
  Edit3, Save, Camera, AlertTriangle, CheckCircle, Loader2,
} from 'lucide-react';

const splitName = (full) => {
  const parts = (full || '').trim().split(/\s+/).filter(Boolean);
  return { firstName: parts[0] || '', lastName: parts.slice(1).join(' ') };
};

const spinKeyframes = `
@keyframes spin {
  to { transform: rotate(360deg); }
}
`;
if (typeof document !== 'undefined' && !document.getElementById('spin-style')) {
  const style = document.createElement('style');
  style.id = 'spin-style';
  style.textContent = spinKeyframes;
  document.head.appendChild(style);
}

const UserProfile = () => {
  const { currentUser, refreshUser } = useUserAuth();
  const { t } = useLanguage();
  const [isEditing, setIsEditing] = useState(false);
  const [showEmergencyModal, setShowEmergencyModal] = useState(false);
  const [saving, setSaving] = useState(false);
  const [savedMsg, setSavedMsg] = useState('');
  const [profilePhoto, setProfilePhoto] = useState(null);
  const [uploadingPhoto, setUploadingPhoto] = useState(false);
  const fileInputRef = useRef(null);

  // Seed from the logged-in account; refined with the full record below.
  const [profile, setProfile] = useState(() => {
    const { firstName, lastName } = splitName(currentUser?.full_name);
    return {
      firstName, lastName,
      email: '', phone: currentUser?.phone || '', birthdate: '',
      gender: '', bloodType: '', address: currentUser?.purok || '',
      civilStatus: '', occupation: '', philhealthNo: '',
    };
  });

  const [emergencyContact, setEmergencyContact] = useState({
    name: '', relationship: '', phone: '',
  });

  const applyData = useCallback((d) => {
    if (!d) return;
    const { firstName, lastName } = splitName(d.full_name);
    setProfile({
      firstName, lastName,
      email: d.email || '',
      phone: d.phone || '',
      birthdate: d.birthdate ? String(d.birthdate).slice(0, 10) : '',
      gender: d.gender || '',
      bloodType: d.blood_type || '',
      address: d.purok || '',
      civilStatus: d.civil_status || '',
      occupation: d.occupation || '',
      philhealthNo: d.philhealth_no || '',
    });
    setEmergencyContact({
      name: d.emergency_name || '',
      relationship: d.emergency_relationship || '',
      phone: d.emergency_phone || '',
    });
    if (d.profile_photo) {
      setProfilePhoto(d.profile_photo);
    }
  }, []);

  useEffect(() => {
    userApi.get('/auth/user/me').then(applyData).catch(() => {});
  }, [applyData]);

  const flashMsg = (m) => { setSavedMsg(m); setTimeout(() => setSavedMsg(''), 3000); };

  const handlePhotoClick = () => {
    if (fileInputRef.current) {
      fileInputRef.current.click();
    }
  };

  const handlePhotoChange = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;

    if (!file.type.startsWith('image/')) {
      flashMsg('Please select an image file.');
      return;
    }

    if (file.size > 2 * 1024 * 1024) {
      flashMsg('Image must be less than 2MB.');
      return;
    }

    setUploadingPhoto(true);
    try {
      const userId = currentUser?.id;
      const ext = file.name.split('.').pop();
      const fileName = `${userId}/profile.${ext}`;

      const { error: uploadError } = await supabase.storage
        .from('profile-photos')
        .upload(fileName, file, { upsert: true, cacheControl: '3600' });

      if (uploadError) throw uploadError;

      const { data: urlData } = supabase.storage
        .from('profile-photos')
        .getPublicUrl(fileName);

      const photoUrl = `${urlData.publicUrl}?t=${Date.now()}`;

      await userApi.patch('/auth/user/me', { profile_photo: photoUrl });
      setProfilePhoto(photoUrl);
      flashMsg('Profile photo updated.');
    } catch (err) {
      console.error('Photo upload error:', err);
      flashMsg('Failed to upload photo.');
    } finally {
      setUploadingPhoto(false);
      if (fileInputRef.current) fileInputRef.current.value = '';
    }
  };

  const handleSave = async () => {
    setSaving(true);
    try {
      const updated = await userApi.patch('/auth/user/me', {
        full_name: `${profile.firstName} ${profile.lastName}`.trim(),
        email: profile.email,
        phone: profile.phone,
        purok: profile.address,
        birthdate: profile.birthdate,
        gender: profile.gender,
        blood_type: profile.bloodType,
        civil_status: profile.civilStatus,
        occupation: profile.occupation,
        philhealth_no: profile.philhealthNo,
      });
      applyData(updated);
      await refreshUser();
      setIsEditing(false);
      flashMsg('Profile updated.');
    } catch (e) {
      flashMsg(e.message || 'Failed to save profile.');
    } finally {
      setSaving(false);
    }
  };

  const handleCancel = () => {
    setIsEditing(false);
    userApi.get('/auth/user/me').then(applyData).catch(() => {});
  };

  const handleEmergencySave = async () => {
    setSaving(true);
    try {
      const updated = await userApi.patch('/auth/user/me', {
        emergency_name: emergencyContact.name,
        emergency_relationship: emergencyContact.relationship,
        emergency_phone: emergencyContact.phone,
      });
      applyData(updated);
      setShowEmergencyModal(false);
      flashMsg('Emergency contact updated.');
    } catch (e) {
      flashMsg(e.message || 'Failed to save contact.');
    } finally {
      setSaving(false);
    }
  };

  const getAge = (birthdate) => {
    const today = new Date();
    const birth = new Date(birthdate);
    let age = today.getFullYear() - birth.getFullYear();
    const m = today.getMonth() - birth.getMonth();
    if (m < 0 || (m === 0 && today.getDate() < birth.getDate())) {
      age--;
    }
    return age;
  };

  return (
    <div>
      {/* Header */}
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        <div>
          <h4 className="fw-bold mb-1" style={{ color: '#111827' }}>{t('myProfile')}</h4>
          <p className="mb-0" style={{ fontSize: 13, color: '#6b7280' }}>
            Manage your personal information and preferences
          </p>
        </div>
        {!isEditing ? (
          <Button
            className="d-flex align-items-center gap-2 border-0"
            style={{ background: 'linear-gradient(135deg, #2563eb, #1d4ed8)', fontSize: 13, fontWeight: 600 }}
            onClick={() => setIsEditing(true)}
          >
            <Edit3 size={16} /> {t('editProfile')}
          </Button>
        ) : (
          <div className="d-flex gap-2">
            <Button
              variant="light"
              className="border"
              style={{ fontSize: 13 }}
              onClick={handleCancel}
              disabled={saving}
            >
              Cancel
            </Button>
            <Button
              className="d-flex align-items-center gap-2 border-0"
              style={{ background: 'linear-gradient(135deg, #2563eb, #1d4ed8)', fontSize: 13, fontWeight: 600 }}
              onClick={handleSave}
              disabled={saving}
            >
              <Save size={16} /> {saving ? 'Saving...' : 'Save Changes'}
            </Button>
          </div>
        )}
      </div>

      {/* Success/Error Message */}
      {savedMsg && (
        <div
          className="d-flex align-items-center gap-2 mb-3 p-3 rounded-3"
          style={{
            backgroundColor: savedMsg.includes('Failed') || savedMsg.includes('failed') ? '#fee2e2' : '#dcfce7',
            color: savedMsg.includes('Failed') || savedMsg.includes('failed') ? '#991b1b' : '#166534',
            fontSize: 13,
            fontWeight: 500,
          }}
        >
          <CheckCircle size={16} />
          {savedMsg}
        </div>
      )}

      <Row className="g-4">
        {/* Profile Card */}
        <Col xs={12} lg={4}>
          <Card className="border rounded-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <Card.Body className="p-4 text-center">
              <div className="position-relative d-inline-block mb-3">
                <input
                  type="file"
                  ref={fileInputRef}
                  onChange={handlePhotoChange}
                  accept="image/*"
                  style={{ display: 'none' }}
                />
                {profilePhoto ? (
                  <img
                    src={profilePhoto}
                    alt="Profile"
                    className="rounded-circle mx-auto"
                    style={{
                      width: 100,
                      height: 100,
                      objectFit: 'cover',
                      border: '3px solid #e5e7eb',
                    }}
                  />
                ) : (
                  <div
                    className="d-flex align-items-center justify-content-center rounded-circle text-white fw-bold mx-auto"
                    style={{
                      width: 100,
                      height: 100,
                      background: 'linear-gradient(135deg, #2563eb, #1d4ed8)',
                      fontSize: 32,
                    }}
                  >
                    {(profile.firstName[0] || '')}{(profile.lastName[0] || '') || (profile.firstName[1] || '')}
                  </div>
                )}
                <button
                  type="button"
                  onClick={handlePhotoClick}
                  disabled={uploadingPhoto}
                  className="position-absolute d-flex align-items-center justify-content-center rounded-circle border-0"
                  style={{
                    width: 32,
                    height: 32,
                    backgroundColor: '#fff',
                    boxShadow: '0 2px 8px rgba(0,0,0,0.15)',
                    bottom: 0,
                    right: 0,
                    cursor: uploadingPhoto ? 'wait' : 'pointer',
                  }}
                >
                  {uploadingPhoto ? (
                    <Loader2 size={16} color="#6b7280" style={{ animation: 'spin 1s linear infinite' }} />
                  ) : (
                    <Camera size={16} color="#6b7280" />
                  )}
                </button>
              </div>

              <h5 className="fw-bold mb-1" style={{ color: '#111827' }}>
                {`${profile.firstName} ${profile.lastName}`.trim() || currentUser?.username || 'User'}
              </h5>
              <p className="mb-3" style={{ fontSize: 13, color: '#6b7280' }}>{profile.email || `@${currentUser?.username || ''}`}</p>

              <div className="d-flex justify-content-center gap-2 flex-wrap">
                {profile.birthdate && (
                  <Badge style={{ backgroundColor: '#dbeafe', color: '#1d4ed8', fontSize: 11 }}>
                    {getAge(profile.birthdate)} years old
                  </Badge>
                )}
                {profile.gender && (
                  <Badge style={{ backgroundColor: '#fce7f3', color: '#be185d', fontSize: 11 }}>
                    {profile.gender}
                  </Badge>
                )}
                {profile.bloodType && (
                  <Badge style={{ backgroundColor: '#fee2e2', color: '#991b1b', fontSize: 11 }}>
                    <Heart size={10} className="me-1" /> {profile.bloodType}
                  </Badge>
                )}
              </div>
            </Card.Body>
          </Card>

          {/* Emergency Contact Card */}
          <Card className="border rounded-4 mt-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <Card.Body className="p-4">
              <div className="d-flex justify-content-between align-items-center mb-3">
                <h6 className="fw-bold mb-0" style={{ fontSize: 14, color: '#111827' }}>
                  <AlertTriangle size={16} className="me-2" style={{ color: '#dc2626' }} />
                  {t('emergencyContactInfo')}
                </h6>
                <Button
                  size="sm"
                  variant="light"
                  className="border-0"
                  style={{ fontSize: 11 }}
                  onClick={() => setShowEmergencyModal(true)}
                >
                  <Edit3 size={12} />
                </Button>
              </div>

              <div className="d-flex flex-column gap-2">
                <div>
                  <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600 }}>NAME</div>
                  <div style={{ fontSize: 13, color: '#374151' }}>{emergencyContact.name || '—'}</div>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600 }}>{t('relationship').toUpperCase()}</div>
                  <div style={{ fontSize: 13, color: '#374151' }}>{emergencyContact.relationship || '—'}</div>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600 }}>PHONE</div>
                  <div style={{ fontSize: 13, color: '#374151' }}>{emergencyContact.phone || '—'}</div>
                </div>
              </div>
            </Card.Body>
          </Card>
        </Col>

        {/* Personal Information */}
        <Col xs={12} lg={8}>
          <Card className="border rounded-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <Card.Body className="p-4">
              <h6 className="fw-bold mb-4" style={{ fontSize: 14, color: '#111827' }}>
                <User size={16} className="me-2" style={{ color: '#1d4ed8' }} />
                {t('personalInfo')}
              </h6>

              <Row className="g-3">
                <Col xs={12} md={6}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>{t('firstName')}</Form.Label>
                    <Form.Control
                      size="sm"
                      value={profile.firstName}
                      onChange={(e) => setProfile({ ...profile, firstName: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    />
                  </Form.Group>
                </Col>
                <Col xs={12} md={6}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>{t('lastName')}</Form.Label>
                    <Form.Control
                      size="sm"
                      value={profile.lastName}
                      onChange={(e) => setProfile({ ...profile, lastName: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    />
                  </Form.Group>
                </Col>
                <Col xs={12} md={6}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>
                      <Mail size={12} className="me-1" /> {t('email')}
                    </Form.Label>
                    <Form.Control
                      type="email"
                      size="sm"
                      value={profile.email}
                      onChange={(e) => setProfile({ ...profile, email: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    />
                  </Form.Group>
                </Col>
                <Col xs={12} md={6}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>
                      <Phone size={12} className="me-1" /> {t('phone')}
                    </Form.Label>
                    <Form.Control
                      size="sm"
                      value={profile.phone}
                      onChange={(e) => setProfile({ ...profile, phone: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    />
                  </Form.Group>
                </Col>
                <Col xs={12} md={6}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>
                      <Calendar size={12} className="me-1" /> {t('birthdate')}
                    </Form.Label>
                    <Form.Control
                      type="date"
                      size="sm"
                      value={profile.birthdate}
                      onChange={(e) => setProfile({ ...profile, birthdate: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    />
                  </Form.Group>
                </Col>
                <Col xs={12} md={6}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>{t('gender')}</Form.Label>
                    <Form.Select
                      size="sm"
                      value={profile.gender}
                      onChange={(e) => setProfile({ ...profile, gender: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    >
                      <option value="">Select Gender</option>
                      <option value="Male">Male</option>
                      <option value="Female">Female</option>
                    </Form.Select>
                  </Form.Group>
                </Col>
                <Col xs={12}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>
                      <MapPin size={12} className="me-1" /> {t('address')}
                    </Form.Label>
                    <Form.Control
                      size="sm"
                      value={profile.address}
                      onChange={(e) => setProfile({ ...profile, address: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    />
                  </Form.Group>
                </Col>
              </Row>
            </Card.Body>
          </Card>

          {/* Medical Information */}
          <Card className="border rounded-4 mt-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <Card.Body className="p-4">
              <h6 className="fw-bold mb-4" style={{ fontSize: 14, color: '#111827' }}>
                <Heart size={16} className="me-2" style={{ color: '#dc2626' }} />
                Medical Information
              </h6>

              <Row className="g-3">
                <Col xs={12} md={4}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>{t('bloodType')}</Form.Label>
                    <Form.Select
                      size="sm"
                      value={profile.bloodType}
                      onChange={(e) => setProfile({ ...profile, bloodType: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    >
                      <option value="">Select Blood Type</option>
                      <option value="A+">A+</option>
                      <option value="A-">A-</option>
                      <option value="B+">B+</option>
                      <option value="B-">B-</option>
                      <option value="AB+">AB+</option>
                      <option value="AB-">AB-</option>
                      <option value="O+">O+</option>
                      <option value="O-">O-</option>
                    </Form.Select>
                  </Form.Group>
                </Col>
                <Col xs={12} md={4}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>{t('civilStatus')}</Form.Label>
                    <Form.Select
                      size="sm"
                      value={profile.civilStatus}
                      onChange={(e) => setProfile({ ...profile, civilStatus: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    >
                      <option value="">Select Status</option>
                      <option value="Single">Single</option>
                      <option value="Married">Married</option>
                      <option value="Widowed">Widowed</option>
                      <option value="Separated">Separated</option>
                    </Form.Select>
                  </Form.Group>
                </Col>
                <Col xs={12} md={4}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>{t('occupation')}</Form.Label>
                    <Form.Control
                      size="sm"
                      value={profile.occupation}
                      onChange={(e) => setProfile({ ...profile, occupation: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    />
                  </Form.Group>
                </Col>
                <Col xs={12} md={6}>
                  <Form.Group>
                    <Form.Label style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>
                      <Shield size={12} className="me-1" /> {t('philhealthNo')}
                    </Form.Label>
                    <Form.Control
                      size="sm"
                      value={profile.philhealthNo}
                      onChange={(e) => setProfile({ ...profile, philhealthNo: e.target.value })}
                      disabled={!isEditing}
                      style={{ fontSize: 13 }}
                    />
                  </Form.Group>
                </Col>
              </Row>
            </Card.Body>
          </Card>
        </Col>
      </Row>

      {/* Emergency Contact Modal */}
      <Modal show={showEmergencyModal} onHide={() => setShowEmergencyModal(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title style={{ fontSize: 16 }}>
            <AlertTriangle size={18} className="me-2" style={{ color: '#dc2626' }} />
            {t('editEmergencyContact')}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Row className="g-3">
            <Col xs={12}>
              <Form.Group>
                <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>Contact Name</Form.Label>
                <Form.Control
                  size="sm"
                  value={emergencyContact.name}
                  onChange={(e) => setEmergencyContact({ ...emergencyContact, name: e.target.value })}
                  style={{ fontSize: 13 }}
                />
              </Form.Group>
            </Col>
            <Col xs={12}>
              <Form.Group>
                <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>{t('relationship')}</Form.Label>
                <Form.Select
                  size="sm"
                  value={emergencyContact.relationship}
                  onChange={(e) => setEmergencyContact({ ...emergencyContact, relationship: e.target.value })}
                  style={{ fontSize: 13 }}
                >
                  <option value="">{t('selectRelationship')}</option>
                  <option value="Spouse">{t('spouse')}</option>
                  <option value="Parent">{t('parent')}</option>
                  <option value="Sibling">{t('sibling')}</option>
                  <option value="Child">{t('child')}</option>
                  <option value="Relative">{t('other')}</option>
                  <option value="Friend">{t('friend')}</option>
                </Form.Select>
              </Form.Group>
            </Col>
            <Col xs={12}>
              <Form.Group>
                <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>Phone Number</Form.Label>
                <Form.Control
                  size="sm"
                  value={emergencyContact.phone}
                  onChange={(e) => setEmergencyContact({ ...emergencyContact, phone: e.target.value })}
                  style={{ fontSize: 13 }}
                />
              </Form.Group>
            </Col>
          </Row>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="light" size="sm" onClick={() => setShowEmergencyModal(false)}>
            Cancel
          </Button>
          <Button
            size="sm"
            className="border-0"
            style={{ background: 'linear-gradient(135deg, #2563eb, #1d4ed8)' }}
            onClick={handleEmergencySave}
          >
            Save Changes
          </Button>
        </Modal.Footer>
      </Modal>
    </div>
  );
};

export default UserProfile;
