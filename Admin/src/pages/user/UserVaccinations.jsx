...
const isOverdue = (nextDue) => {
  ...
  const due = new Date(String(nextDue).slice(0, 10) + 'T00:00:00'); // local midnight
...
const UserVaccinations = () => {
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-4">
        <h4 className="fw-bold mb-1" style={{ color: '#111827' }}>{t('myVaccinations')}</h4>
                    ...
                    return (
                      <div key={vax.id} className="p-3 rounded-3"
                        ...
                        <Button size="sm" className="w-100 border-0"
                          ...
                          onClick={() => navigate('/user/appointments')}>
                          {overdue ? '⚠ ' + t('scheduleImmediately') : t('scheduleAppointment')} <ChevronRight size={13} />
                        </Button>
                      </div>
                    );
                  })}
          ...
          <Card className="border rounded-4 h-100" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <Card.Body className="p-4">
              <h6 className="fw-bold mb-3" style={{ fontSize: 14, color: '#111827' }}>
                ...
                {t('vaccinationHistory')} ({completed.length} {t('completed')})
              </h6>
              {loading ? (
                <div style={{ fontSize: 13, color: '#9ca3af', textAlign: 'center', padding: '30px 0' }}>Loading…</div>
              ) : completed.length === 0 ? (
