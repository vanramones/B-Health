...
const StatusPill = ({ status }) => {
  ...
  return (
    <span style={{
      ...
      backgroundColor: sv.bg, color: sv.color, border: `1px solid ${sv.border}`,
...
const emptyForm = {
  user_id: '', patient: '', age: '', vaccine: 'BCG',
  dose: '1st Dose', date: '', next_due: '',
  administered_by: '', site: 'Left arm (deltoid)', status: 'scheduled', notes: '',
};
...
const Vaccination = () => {
  ...
  const openEdit = (r) => {
    ...
    setForm({
      ...
      patient: r.patient || '',
      age: String(r.age || ''),
      vaccine: r.vaccine || 'BCG',
      dose: r.dose || '1st Dose',
      date: r.date || '',
      next_due: r.next_due || '',
      administered_by: r.administered_by || '',
      site: r.site || 'Left arm (deltoid)',
    ...
    <div className="p-4" style={{ backgroundColor: '#f8fafc' }}>
      ...
      <Row className="g-2 g-md-3 mb-4 bh-stagger">
        {summaryCards.map((c) => (
          <Col key={c.key} xs={6} lg={3}>
            <Card className="border rounded-4 bh-card-hover"
          ...
          <div className="d-flex flex-wrap align-items-center gap-2">
            ...
            <div className="ms-auto d-flex gap-2 mt-2 mt-md-0">
              <InputGroup size="sm" style={{ maxWidth: 260, minWidth: 130 }}>
                ...
                <Form.Control className="bg-light border-start-0" placeholder="Search patient, vaccine…"
                  value={query} onChange={(e) => setQuery(e.target.value)} style={{ fontSize: 12 }} />
              </InputGroup>
              <Button size="sm" onClick={openAdd} className="d-flex align-items-center gap-1 border-0 flex-shrink-0"
                style={{ backgroundColor: '#16a34a', fontSize: 12, fontWeight: 600 }}>
            ...
            <Table hover className="mb-0 align-middle">
              <thead style={{ backgroundColor: '#f9fafb' }}>
                <tr style={{ fontSize: 11, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                  ...
                  <th className="py-3">Dose</th>
                  <th className="py-3">Date Given</th>
                  <th className="py-3">Next Due</th>
                  <th className="py-3">Given By</th>
              ...
              <tbody style={{ fontSize: 13 }}>
                {loading && (
                  <tr><td colSpan={8} className="text-center py-5" style={{ color: '#9ca3af' }}>Loading…</td></tr>
                )}
                {!loading && filtered.length === 0 && (
                  <tr><td colSpan={8} className="text-center py-5" style={{ color: '#9ca3af' }}>No vaccination records found.</td></tr>
                )}
                {filtered.map((r) => (
                  <tr key={r.id} style={{ cursor: 'pointer' }}
                      ...
                      <div className="d-flex align-items-center gap-2">
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: '#9ca3af' }}>
                            {r.age ? `${r.age} y/o` : ''}
                            {r.user_id ? <span className="ms-1" style={{ color: '#16a34a' }}>● registered</span> : ''}
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3 text-end pe-4" onClick={(e) => e.stopPropagation()}>
                      <div className="d-flex justify-content-end gap-1">
                        {r.status === 'scheduled' && (
                          <Button size="sm" variant="light" className="border-0 p-1 px-2" title="Mark Completed"
                            onClick={() => quickStatus(r.id, 'completed')}>
                            <CheckCircle size={14} color="#16a34a" />
                          </Button>
                        )}
                        {r.status === 'scheduled' && (
                          <Button size="sm" variant="light" className="border-0 p-1 px-2" title="Mark Missed"
                            onClick={() => quickStatus(r.id, 'missed')}>
                            <XCircle size={14} color="#dc2626" />
                          </Button>
                        )}
                        <Button size="sm" variant="light" className="border-0 p-1 px-2" title="Edit"
                          onClick={() => openEdit(r)}>
                          <Edit3 size={14} color="#1d4ed8" />
                        </Button>
                        <Button size="sm" variant="light" className="border-0 p-1 px-2" title="Delete"
      ...
      <Modal show={showForm} onHide={closeForm} centered size="lg">
        <Modal.Header closeButton style={{ borderBottom: '2px solid #dcfce7' }}>
          <Modal.Title style={{ fontSize: 16, fontWeight: 700 }}>
            <Syringe size={17} className="me-2" color="#16a34a" />
            {editingId ? 'Edit Vaccination Record' : 'Record Vaccination'}
          </Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleSave}>
          <Modal.Body className="p-4">
            ...
            <div className="p-3 rounded-3 mb-3" style={{ backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0' }}>
              <Form.Label style={{ fontSize: 12, fontWeight: 700, color: '#15803d', marginBottom: 6 }}>
                <Users size={13} className="me-1" /> Link to Registered User (optional)
              </Form.Label>
              <Form.Select size="sm" value={form.user_id} onChange={(e) => selectUser(e.target.value)}
                style={{ fontSize: 13 }}>
              ...
              <div style={{ fontSize: 11, color: '#16a34a', marginTop: 4 }}>
                Selecting a user links this record to their account so they can view it.
              </div>
            </div>
            ...
            <Row className="g-3 mb-3">
              <Col xs={12} md={8}>
                ...
                <Form.Control size="sm" value={form.patient}
                  onChange={(e) => setForm({ ...form, patient: e.target.value })} required style={{ fontSize: 13 }} />
              </Col>
              <Col xs={12} md={4}>
                <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>Age</Form.Label>
                <Form.Control type="number" min={0} max={130} size="sm" value={form.age}
                  onChange={(e) => setForm({ ...form, age: e.target.value })} style={{ fontSize: 13 }} />
              </Col>
            </Row>
            ...
            <Row className="g-3 mb-3">
              <Col xs={12} md={6}>
                ...
                <Form.Select size="sm" value={form.vaccine}
                  onChange={(e) => setForm({ ...form, vaccine: e.target.value })} style={{ fontSize: 13 }}>
                  {Object.keys(VACCINE_COLORS).map((v) => <option key={v}>{v}</option>)}
                </Form.Select>
              </Col>
              <Col xs={12} md={6}>
                <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>Dose *</Form.Label>
                <Form.Select size="sm" value={form.dose}
                  onChange={(e) => setForm({ ...form, dose: e.target.value })} style={{ fontSize: 13 }}>
            ...
            <Row className="g-3 mb-3">
              <Col xs={12} md={6}>
                ...
                <Form.Control type="date" size="sm" value={form.date}
                  onChange={(e) => setForm({ ...form, date: e.target.value })} required style={{ fontSize: 13 }} />
              </Col>
              <Col xs={12} md={6}>
                <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>Next Due Date</Form.Label>
                <Form.Control type="date" size="sm" value={form.next_due}
                  onChange={(e) => setForm({ ...form, next_due: e.target.value })} style={{ fontSize: 13 }} />
              </Col>
            </Row>
            ...
            <Row className="g-3 mb-3">
              <Col xs={12} md={7}>
                ...
                <Form.Control size="sm" placeholder="e.g. Nurse Cruz, Dr. Santos"
                  value={form.administered_by}
                  onChange={(e) => setForm({ ...form, administered_by: e.target.value })} style={{ fontSize: 13 }} />
              </Col>
              <Col xs={12} md={5}>
                <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>Injection Site</Form.Label>
                <Form.Select size="sm" value={form.site}
                  onChange={(e) => setForm({ ...form, site: e.target.value })} style={{ fontSize: 13 }}>
            ...
            <Row className="g-3">
              <Col xs={12} md={4}>
                ...
                <Form.Select size="sm" value={form.status}
                  onChange={(e) => setForm({ ...form, status: e.target.value })} style={{ fontSize: 13 }}>
              ...
              <Col xs={12} md={8}>
                ...
                <Form.Control size="sm" placeholder="Adverse reactions, observations…"
                  value={form.notes}
                  onChange={(e) => setForm({ ...form, notes: e.target.value })} style={{ fontSize: 13 }} />
              </Col>
            </Row>
          </Modal.Body>
          <Modal.Footer>
            <Button variant="light" size="sm" className="border" onClick={closeForm}>Cancel</Button>
            <Button type="submit" size="sm" className="border-0" disabled={saving}
            ...
            <>
              ...
              <Modal.Footer className="justify-content-between">
                <div className="d-flex gap-2">
                  {showView.status === 'scheduled' && (
                    <Button size="sm" className="border-0" style={{ backgroundColor: '#16a34a', fontSize: 12 }}
                      onClick={() => { quickStatus(showView.id, 'completed'); setShowView(null); }}>
                      <CheckCircle size={13} className="me-1" /> Mark Completed
                    </Button>
                  )}
                  {showView.status === 'scheduled' && (
                    <Button size="sm" variant="light" className="border" style={{ fontSize: 12, color: '#dc2626' }}
                      onClick={() => { quickStatus(showView.id, 'missed'); setShowView(null); }}>
                      Mark Missed
                    </Button>
                  )}
                </div>
      ...
      <Modal show={!!confirmDelete} onHide={() => setConfirmDelete(null)} centered size="sm">
        ...
        <Modal.Body style={{ fontSize: 13 }}>
          Delete <strong>{confirmDelete?.vaccine}</strong> ({confirmDelete?.dose}) record for <strong>{confirmDelete?.patient}</strong>? This cannot be undone.
        </Modal.Body>
        <Modal.Footer>
