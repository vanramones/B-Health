...
const UserAppointments = () => {
  ...
  useEffect(() => {
    const handleAppointmentUpdate = (data) => {
      console.log('[Socket] Appointment status update:', data);
      setNotification({
        type: data.status,
  ...
  return (
    <div>
      ...
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        <div>
          <h4 className="fw-bold mb-1" style={{ color: '#111827' }}>My Appointments</h4>
            ...
            return (
              <Col key={apt.id} xs={12} md={6} lg={4}>
                <Card
                  ...
                  style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)', cursor: 'pointer', borderLeft: `4px solid ${sc.color} !important`, transition: 'box-shadow 0.15s' }}
            ...
            <>
              ...
              <Modal.Body style={{ padding: '20px', maxHeight: '70vh', overflowY: 'auto' }}>
                ...
                <div style={{
                  ...
                  marginBottom: 16,
                  borderRadius: 16,
                  background: `linear-gradient(135deg, ${sc.bg}, ${sc.bg}cc)`,
                }}>
                  <div style={{
                    width: 64, height: 64, borderRadius: '50%',
                    backgroundColor: '#fff', margin: '0 auto 12px',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    boxShadow: `0 4px 16px ${sc.color}33`,
                    color: sc.color,
                  }}>
                  ...
                  <span style={{
                    ...
                    backgroundColor: '#fff', color: sc.color,
                  ...
                  {[
                    {
                      ...
                      label: 'Date',
                      value: new Date(viewApt.date).toLocaleDateString('en-US', {
                    ...
                    viewApt.status !== 'pending' && {
                      ...
                      value: viewApt.handled_by || 'Admin',
                    },
                    {
                      icon: <Clock size={18} color="#6b7280" />,
                      label: 'Booked On',
                      value: new Date(viewApt.created_at).toLocaleDateString('en-US', {
                    ...
                    <div key={i} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 10, color: '#9ca3af', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>
                          {row.label}
                        </div>
                        <div style={{ fontSize: 14, fontWeight: 600, color: '#111827', wordBreak: 'break-word', lineHeight: 1.4 }}>
                          {row.value}
                        </div>
                      </div>
                    </div>
              ...
              <Modal.Footer style={{ padding: '12px 20px', gap: 8, flexWrap: 'wrap', borderTop: '1px solid #f3f4f6' }}>
                ...
                {viewApt.status === 'pending' && !isRescheduled(viewApt) && (
                  <Button
                    ...
                    disabled={cancelling === viewApt.id}
                    onClick={() => { handleCancel(viewApt.id); setViewApt(null); }}
                  >
                    {cancelling === viewApt.id ? 'Cancelling…' : 'Cancel Appointment'}
                  </Button>
                )}
                <Button
                  size="sm" variant="secondary" className="flex-fill"
                  style={{ fontSize: 13, fontWeight: 600 }}
        ...
        <Form onSubmit={handleBookingSubmit}>
          <Modal.Body>
                ...
                <Form.Group>
                  <Form.Label style={{ fontSize: 12, fontWeight: 600 }}>
                    ...
                    {bookingForm.date && bookingForm.service && !slotLoading && (
                      <span className="ms-2" style={{ fontSize: 11, color: '#6b7280', fontWeight: 400 }}>
                        ({slotLimit} slots/hour)
                      </span>
                    )}
                  </Form.Label>
                  <Form.Select
            ...
            <div
              ...
              style={{ backgroundColor: '#eff6ff', border: '1px solid #bfdbfe', fontSize: 12, color: '#1d4ed8' }}
            >
          ...
          <Modal.Footer>
            <Button variant="light" size="sm" onClick={() => setShowBooking(false)}>
              Cancel
            </Button>
            <Button type="submit" size="sm" className="border-0"
